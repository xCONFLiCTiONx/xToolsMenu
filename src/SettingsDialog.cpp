...
HWND g_hBtnAdd = nullptr, g_hBtnDel = nullptr;
...
void SaveCustomCommand(const wchar_t* name, const wchar_t* path, const wchar_t* args) {
    HKEY hKey;
...
void DeleteCustomCommand(const wchar_t* name) {
    HKEY hKey;
...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CTLCOLORLISTBOX: {
        ...
        if (uMsg == WM_CTLCOLOREDIT || uMsg == WM_CTLCOLORLISTBOX) {
            ...
            static HBRUSH hbrEdit = CreateSolidBrush(DARK_CONTROL_BACK);
    ...
    case WM_CREATE: {
        ...
        g_hTab = CreateWindowW(WC_TABCONTROLW, L"", WS_CHILD | WS_VISIBLE | TCS_OWNERDRAWFIXED, 10, 10, 380, 440, hwnd, NULL, hInst, NULL);
        ...
        SendMessage(g_hListCustom, WM_SETFONT, (WPARAM)g_hFont, TRUE);
        ...
        g_hEditName = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_BORDER, 80, y, 150, 25, hwnd, NULL, hInst, NULL);
        y += 30;
        g_hStaticPath = CreateWindowW(L"STATIC", L"Path:", WS_CHILD, 25, y, 50, 25, hwnd, NULL, hInst, NULL);
        g_hEditPath = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_BORDER, 80, y, 200, 25, hwnd, NULL, hInst, NULL);
        y += 30;
        g_hStaticArgs = CreateWindowW(L"STATIC", L"Args:", WS_CHILD, 25, y, 50, 25, hwnd, NULL, hInst, NULL);
        g_hEditArgs = CreateWindowExW(WS_EX_CLIENTEDGE, L"EDIT", L"", WS_CHILD | WS_BORDER, 80, y, 200, 25, hwnd, NULL, hInst, NULL);

        g_hBtnAdd = CreateWindowW(L"BUTTON", L"Add", WS_CHILD | BS_OWNERDRAW, 290, 260, 80, 25, hwnd, (HMENU)100, hInst, NULL);
        g_hBtnDel = CreateWindowW(L"BUTTON", L"Delete", WS_CHILD | BS_OWNERDRAW, 290, 290, 80, 25, hwnd, (HMENU)101, hInst, NULL);
    ...
    case WM_NOTIFY: {
        ...
        if (nmhdr->code == TCN_SELCHANGE) UpdateTabVisibility();
        return 0;
    }
    case WM_COMMAND: {
        ...
        if (HIWORD(wParam) == BN_CLICKED) {
            ...
            if (wmId == 100) { // Add
                ...
                if (wcslen(name) > 0 && wcslen(path) > 0) {
                    SaveCustomCommand(name, path, args);
                    LoadCustomCommands();
                }
                ...
                auto UpdateGroup = [&](std::vector<SettingItem>& group) {
                    for (auto& item : group) if (item.hWnd == hCtrl) { SetSetting(item.regValue.c_str(), checked); break; }
                };
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, int nCmdShow) {
    ...
    RegisterClassExW(&wc);
    HWND hwnd = CreateWindowExW(0, CLASS_NAME, L"xToolsMenu Settings", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU, CW_USEDEFAULT, CW_USEDEFAULT, 420, 500, NULL, NULL, hInstance, NULL);
    if (!hwnd) return 0;
